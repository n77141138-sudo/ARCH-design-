...
if selected_category == "預售客變":
    ...
    if current_info and "floorplan_path" in current_info:
        fp_path = BASE_DIR / current_info["floorplan_path"]
        
        col_img, col_ai = st.columns(2)
        with col_img:
            st.write(f"**客戶：{current_info.get('client_name')} (Tel: {current_info.get('phone')})**")
            if fp_path.exists():
